...
class Config:
    ...
    # ── AT3: Multi-Setup Engine (disabled by default)
    AT3_ENABLED = env_bool("DALVAX_AT3_ENABLED", False)

    # Individual setups (only used if AT3_ENABLED=true)
    SETUP_MR_ENABLED = env_bool("DALVAX_SETUP_MR_ENABLED", True)  # existing topo/fundo (mean-reversion)
    SETUP_TREND_BREAKOUT_ENABLED = env_bool("DALVAX_SETUP_TREND_BREAKOUT_ENABLED", False)
    SETUP_TREND_PULLBACK_ENABLED = env_bool("DALVAX_SETUP_TREND_PULLBACK_ENABLED", False)
    SETUP_VOL_SQUEEZE_ENABLED = env_bool("DALVAX_SETUP_VOL_SQUEEZE_ENABLED", False)

    # Shared knobs
    SETUP_TIMEFRAME = os.getenv("DALVAX_SETUP_TIMEFRAME", "").strip()  # optional override (e.g., "5m", "15m")
    SETUP_SCORE_MIN = env_float("DALVAX_SETUP_SCORE_MIN", 0.0)

    # Trend Breakout
    TB_ADX_MIN = env_float("DALVAX_TB_ADX_MIN", 18.0)
    TB_EMA_FAST = env_int("DALVAX_TB_EMA_FAST", 10)
    TB_EMA_SLOW = env_int("DALVAX_TB_EMA_SLOW", 20)
    TB_BB_PERIOD = env_int("DALVAX_TB_BB_PERIOD", 20)
    TB_BB_STD = env_float("DALVAX_TB_BB_STD", 2.0)

    # Trend Pullback
    TP_REGIME_EMA = env_int("DALVAX_TP_REGIME_EMA", 50)
    TP_PULLBACK_EMA = env_int("DALVAX_TP_PULLBACK_EMA", 10)
    TP_PULLBACK_ATR_MULT = env_float("DALVAX_TP_PULLBACK_ATR_MULT", 0.35)
    TP_RSI_MIN_LONG = env_float("DALVAX_TP_RSI_MIN_LONG", 52.0)
    TP_RSI_MAX_SHORT = env_float("DALVAX_TP_RSI_MAX_SHORT", 48.0)

    # Volatility Squeeze
    VS_BB_PERIOD = env_int("DALVAX_VS_BB_PERIOD", 20)
    VS_BB_STD = env_float("DALVAX_VS_BB_STD", 2.0)
    VS_BB_WIDTH_MAX = env_float("DALVAX_VS_BB_WIDTH_MAX", 0.045)  # ~4.5%
    VS_LOOKBACK = env_int("DALVAX_VS_LOOKBACK", 12)
...
# ── UI Colors (AT4) ──────────────────────────────────────────────────────────
# Mapeia regimes/setups para cores (usado no /dashboard e /api/public_status).
REGIME_COLORS = {
    "DAY":      "#FFD54F",  # 🟡 amarelo (mean-reversion / range)
    "POSITION": "#66BB6A",  # 🟢 verde (trend-follow)
    "UNKNOWN":  "#90A4AE",  # cinza
}

SETUP_COLORS = {
    "MR":             "#FFD54F",  # 🟡
    "TREND_BREAKOUT": "#EF5350",  # 🔴
    "TREND_PULLBACK": "#42A5F5",  # 🔵
    "VOL_SQUEEZE":    "#AB47BC",  # 🟣
    "UNKNOWN":        "#90A4AE",
}

def color_for_regime(regime: str) -> str:
    return REGIME_COLORS.get((regime or "UNKNOWN").upper(), REGIME_COLORS["UNKNOWN"])

def color_for_setup(setup: str) -> str:
    key = (setup or "UNKNOWN").upper()
    return SETUP_COLORS.get(key, SETUP_COLORS["UNKNOWN"])
...
class Indicators:
    ...
    @staticmethod
    def bollinger(values: list, period: int, std_dev: float):
        """Compat wrapper para AT3.
        Retorna (mid, upper, lower, width) onde width = (upper-lower)/mid.
        """
        if not values or len(values) < period:
            return None
        mid, ub, lb = Indicators.bb(values, period, std_dev)
        if mid is None or ub is None or lb is None or mid == 0:
            return None
        width = (ub - lb) / mid
        return mid, ub, lb, width

    @staticmethod
    def atr(highs: list, lows: list, closes: list, period: int = 14) -> float | None:
        """ATR Wilder a partir de arrays (high, low, close). Compat com AT3."""
        try:
    ...
    def macd(values, fast=12, slow=26, signal=9):
        """Return (macd_line, signal_line, hist) for the LAST point."""
        if not values or len(values) < slow + signal + 2:
            return None, None, None
        ...
        if len(macd_clean) < signal + 2:
            return None, None, None

        sig_series = ema_series(macd_clean, signal)
        sig_last = sig_series[-1]
        macd_last = macd_clean[-1]
        hist_last = (macd_last - sig_last) if sig_last is not None else None
...
# [EXIT-01] Rastreamento de posições abertas para detectar fechamentos por TP/SL
        ...
        # ── AT4 UI: estado atual para cor do dashboard ───────────────
        self._ui_regime: str = self._dual_mode
        self._ui_setup:  str = "MR"
        self._ui_setup_meta: dict = {}
    ...
    async def _get_signal(self, instId):
        ...
        if signal is None:
            # AT3 disabled -> keep legacy behavior
            if not getattr(self.config, 'AT3_ENABLED', False):
                return None, None

            # AT3 enabled -> tenta setups adicionais (TREND_BREAKOUT / PULLBACK / VOL_SQUEEZE)
            highs = [float(x[2]) for x in c5]
            lows  = [float(x[3]) for x in c5]
            try:
                at3_sig, at3_meta = self._select_at3_setup(closes, highs, lows)
            except Exception as _e:
                at3_sig, at3_meta = None, {"error": f"AT3_SELECT:{_e}", "setup": "UNKNOWN"}

            if not at3_sig:
                return None, None

            signal = at3_sig
            meta   = dict(at3_meta or {})
            self._ui_setup_meta = dict(meta)
            # Atualiza setup atual (para UI/cores)
            try:
                self._ui_setup = (meta.get("setup") or "UNKNOWN").upper()
            except Exception:
                self._ui_setup = "UNKNOWN"
        ...
        # AT4: incluir setup/regime para UI/cores (e preservar meta AT3 se existir)
        try:
            meta["regime"] = getattr(self, "_dual_mode", "UNKNOWN")
            meta["regime_color"] = color_for_regime(meta["regime"])
            su = getattr(self, "_ui_setup", "UNKNOWN")
            meta["setup"] = su
            meta["setup_color"] = color_for_setup(su)
            if getattr(self, "_ui_setup_meta", None):
                # não sobrescreve chaves já presentes em meta base
                for k, v in dict(self._ui_setup_meta).items():
                    meta.setdefault(k, v)
        except Exception:
            pass
    ...
    async def _update_dual_mode(self) -> None:
        ...
        if self._dual_confirm >= max(1, self.config.REGIME_CONFIRM_CYCLES):
            ...
            if prev != self._dual_mode:
                ...
                # AT4 UI: atualiza cor/regime no dashboard
                self._ui_regime = self._dual_mode
        else:
            logger.debug(
            ...
            try:
                ...
                try:
                    ...
                    with _STATE_LOCK:
                        ...
                        _STATE["regime"]       = getattr(self, "_dual_mode", "UNKNOWN")
                        _STATE["regime_color"] = color_for_regime(_STATE["regime"])
                        _STATE["setup"]        = getattr(self, "_ui_setup", "UNKNOWN")
                        _STATE["setup_color"]  = color_for_setup(_STATE["setup"])
...
_STATE = {
    ...
    "regime":           "UNKNOWN",
    "regime_color":     color_for_regime("UNKNOWN"),
    "setup":            "UNKNOWN",
    "setup_color":      color_for_setup("UNKNOWN"),
}
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/":
            # Página simples (sem auth) que muda a cor conforme regime/setup
            html = """<!doctype html>
<html>
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>DALVAX Service</title>
<style>
  html,body{{height:100%;margin:0;font-family:system-ui, -apple-system, Segoe UI, Roboto, Arial;}}
  body{{display:flex;align-items:center;justify-content:center;transition:background 0.4s ease;}}
  .card{{background:rgba(255,255,255,0.88);backdrop-filter: blur(6px);
        border-radius:18px; padding:20px 18px; width:min(520px, 92vw);
...
<body>
  <div class="card">
    <div class="row"><div class="k">Bot</div><div class="v">DALVAX v3.1.3 AT4 Colors</div></div>
    <div class="row"><div class="k">Run</div><div class="v" id="run">—</div></div>
    <div class="row"><div class="k">Regime</div><div class="v"><span class="pill" id="reg">—</span></div></div>
    <div class="row"><div class="k">Setup</div><div class="v"><span class="pill" id="setup">—</span></div></div>
    <div class="row"><div class="k">Equity</div><div class="v" id="eq">—</div></div>
    <div class="row"><div class="k">Daily PnL</div><div class="v" id="dp">—</div></div>
...
async function tick(){
  try{
    ...
    document.body.style.background = j.regime_color || '#90A4AE';
    document.getElementById('run').textContent = j.run_id || '—';
    const reg = document.getElementById('reg');
    reg.textContent = j.regime || '—';
    reg.style.background = j.regime_color || '#90A4AE';
    const setup = document.getElementById('setup');
    setup.textContent = j.setup || '—';
    setup.style.background = j.setup_color || '#90A4AE';
    document.getElementById('eq').textContent = (j.equity!=null)? j.equity.toFixed(2) : '—';
    document.getElementById('dp').textContent = (j.daily_pnl!=null)? j.daily_pnl.toFixed(2) : '—';
    document.getElementById('st').textContent = (j.running? 'RUNNING' : 'IDLE');
  }catch(e){}
}
tick();
setInterval(tick, 2000);
        ...
        if p == "/api/public_status":
            s = _STATE
            dopen = s.get("daily_open") or s.get("equity")
            dpnl  = (s.get("equity") or 0) - (dopen or 0) if dopen else 0
            return self._json({
                "ok": True,
                "version": "3.1.3-at4",
                "run_id": RUN_ID,
                ...
                "daily_pnl": float(dpnl or 0.0),
                "regime": s.get("regime","UNKNOWN"),
                "regime_color": s.get("regime_color", color_for_regime("UNKNOWN")),
                "setup": s.get("setup","UNKNOWN"),
                "setup_color": s.get("setup_color", color_for_setup("UNKNOWN")),
            })
        if p == "/api/health":
...
# ─────────────────────────────────────────────────────────────────────────────
# AT3: Multi-Setup Engine (optional)
# Each setup returns (signal, score, meta) where signal in {"buy","sell"}.
# The existing invert/AT1 logic is applied later in the pipeline (kept intact).
# ─────────────────────────────────────────────────────────────────────────────

def _setup_trend_breakout(self, closes, highs, lows):
    try:
        cfg = self.config
        bb = Indicators.bollinger(closes, cfg.TB_BB_PERIOD, cfg.TB_BB_STD)
        if not bb:
            return None, 0.0, {}
        mid, ub, lb, width = bb
        adx = Indicators.adx(highs, lows, closes, 14)
        if adx is None or adx < cfg.TB_ADX_MIN:
            return None, 0.0, {"adx": adx, "bb_width": width}

        ema_fast = Indicators.ema(closes, cfg.TB_EMA_FAST)
        ema_slow = Indicators.ema(closes, cfg.TB_EMA_SLOW)
        ...
        if c_prev <= ub and c > ub and ema_fast > ema_slow:
            score = float(min(5.0, (adx - cfg.TB_ADX_MIN) / 10.0 + 1.0))
            return "buy", score, {"setup": "TREND_BREAKOUT", "adx": adx, "bb_width": width}
        if c_prev >= lb and c < lb and ema_fast < ema_slow:
            score = float(min(5.0, (adx - cfg.TB_ADX_MIN) / 10.0 + 1.0))
            return "sell", score, {"setup": "TREND_BREAKOUT", "adx": adx, "bb_width": width}

        return None, 0.0, {"adx": adx, "bb_width": width}
    except Exception as e:
        return None, 0.0, {"error": f"TB:{e}"}

def _setup_trend_pullback(self, closes, highs, lows):
    try:
        cfg = self.config
        atr = Indicators.atr(highs, lows, closes, 14)
        if atr is None:
            return None, 0.0, {}
        ema_reg = Indicators.ema(closes, cfg.TP_REGIME_EMA)
        ema_pb = Indicators.ema(closes, cfg.TP_PULLBACK_EMA)
        if ema_reg is None or ema_pb is None:
            return None, 0.0, {}

        rsi = Indicators.rsi(closes, 14)
        c = closes[-1]
        dist = abs(c - ema_pb)
        tol = float(cfg.TP_PULLBACK_ATR_MULT) * float(atr)

        if c > ema_reg and dist <= tol and rsi is not None and rsi >= cfg.TP_RSI_MIN_LONG:
            score = float(min(5.0, 1.0 + (rsi - cfg.TP_RSI_MIN_LONG) / 20.0))
            return "buy", score, {"setup": "TREND_PULLBACK", "rsi": rsi, "atr": atr}
        if c < ema_reg and dist <= tol and rsi is not None and rsi <= cfg.TP_RSI_MAX_SHORT:
            score = float(min(5.0, 1.0 + (cfg.TP_RSI_MAX_SHORT - rsi) / 20.0))
            return "sell", score, {"setup": "TREND_PULLBACK", "rsi": rsi, "atr": atr}

        return None, 0.0, {"rsi": rsi, "atr": atr}
    except Exception as e:
        return None, 0.0, {"error": f"TP:{e}"}

def _setup_vol_squeeze(self, closes, highs, lows):
    try:
        cfg = self.config
        bb = Indicators.bollinger(closes, cfg.VS_BB_PERIOD, cfg.VS_BB_STD)
        if not bb:
            return None, 0.0, {}
        mid, ub, lb, width = bb
        if width is None:
        ...
        widths = []
        for i in range(-cfg.VS_LOOKBACK, 0):
            if len(closes) + i <= cfg.VS_BB_PERIOD:
                continue
            w = Indicators.bollinger(closes[:i], cfg.VS_BB_PERIOD, cfg.VS_BB_STD)
            if w:
                widths.append(w[3])

        is_squeeze = (width <= cfg.VS_BB_WIDTH_MAX) and (len(widths) >= 3 and max(widths) <= cfg.VS_BB_WIDTH_MAX * 1.2)
        if not is_squeeze:
            return None, 0.0, {"bb_width": width}

        c = closes[-1]
        c_prev = closes[-2] if len(closes) >= 2 else c
        macd, sig, hist = Indicators.macd(closes, 12, 26, 9)

        if c_prev <= ub and c > ub and (hist is None or hist > 0):
            score = 2.0 if hist is None else float(min(5.0, 2.0 + abs(hist) / 0.0001))
            return "buy", score, {"setup": "VOL_SQUEEZE", "bb_width": width, "hist": hist}
        if c_prev >= lb and c < lb and (hist is None or hist < 0):
            score = 2.0 if hist is None else float(min(5.0, 2.0 + abs(hist) / 0.0001))
            return "sell", score, {"setup": "VOL_SQUEEZE", "bb_width": width, "hist": hist}

        return None, 0.0, {"bb_width": width, "hist": hist}
    except Exception as e:
        return None, 0.0, {"error": f"VS:{e}"}

def _select_at3_setup(self, closes, highs, lows):
    """Return best (signal, meta) using enabled setups. None if no setup triggers."""
    cfg = self.config
    candidates = []

    if cfg.SETUP_TREND_BREAKOUT_ENABLED:
        s, score, meta = self._setup_trend_breakout(closes, highs, lows)
        if s and score >= cfg.SETUP_SCORE_MIN:
            candidates.append((score, s, meta))
    if cfg.SETUP_TREND_PULLBACK_ENABLED:
        s, score, meta = self._setup_trend_pullback(closes, highs, lows)
        if s and score >= cfg.SETUP_SCORE_MIN:
            candidates.append((score, s, meta))
    if cfg.SETUP_VOL_SQUEEZE_ENABLED:
        s, score, meta = self._setup_vol_squeeze(closes, highs, lows)
        if s and score >= cfg.SETUP_SCORE_MIN:
            candidates.append((score, s, meta))
    ...
    score, signal, meta = candidates[0]
    meta = dict(meta or {})
    meta["at3_score"] = score
    return signal, meta



# ── AT3 (patch): ligar helpers de setup ao Bot ───────────────────────────────
try:
    Bot._setup_trend_breakout = _setup_trend_breakout
    Bot._setup_trend_pullback = _setup_trend_pullback
    Bot._setup_vol_squeeze = _setup_vol_squeeze
    Bot._select_at3_setup = _select_at3_setup
except Exception:
    pass

# ── HTTP server bootstrap (AT4.1) ────────────────────────────────────────────
# Importar este módulo NÃO deve iniciar um servidor HTTP automaticamente.
# O servidor deve iniciar apenas quando executado como script (__main__),
# para evitar duplicação em hot-reload e permitir testes via import.
_api_thread = None
...
def start_api_server():
    """Inicia o servidor HTTP em background (idempotente)."""
    global _api_thread
    try:
        ...
        pass
    _api_thread = threading.Thread(target=_start_api_server, name="dalvax-http", daemon=True)
    _api_thread.start()
