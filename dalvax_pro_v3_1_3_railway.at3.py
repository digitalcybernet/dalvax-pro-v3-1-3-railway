    ...
    async def _get_signal(self, instId):
        ...
        if signal is None:
            ...
            if not getattr(self.config, 'AT3_ENABLED', False):
                return None, None
            # AT3 enabled -> we may still produce a signal via other setups
            pass
...
def _select_at3_setup(self, closes, highs, lows):
    ...
    if not candidates:
        ...
        # ── AT3: try additional setups if legacy MR produced no signal
        if getattr(self.config, "AT3_ENABLED", False):
            try:
                at3_sig, at3_meta = self._select_at3_setup(closes, highs, lows)
            except Exception as _e:
                at3_sig, at3_meta = None, {"error": f"AT3_SELECT:{_e}"}
            if at3_sig:
                reason = f"AT3:{(at3_meta or {}).get('setup','UNKNOWN')}"
                return at3_sig, (at3_meta or {"setup": "AT3"})
        return None, None
